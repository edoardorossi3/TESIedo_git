...
def spectre(suffix_file, age50_min, age50_max, dage_n_max=None):
    ...
    ax1.set_ylabel(r'$L_{\lambda}/L_{\lambda,5500}$', size=20)
...
def spec_indices_articolo(suffix_file, age50_min, age50_max, wl_range1, wl_range2, wl_range3, wl_range4, wl_range5, wl_range6, wl_norm1, wl_norm2, wl_norm3,wl_norm4,wl_norm5,wl_norm6, title1='', title2='', title3='', title4='', title5='', title6=''):
    import numpy as np
    import astropy.io.fits as fits
    import os
    import matplotlib.pyplot as plt
    ...
    sel_wl3=((wl>wl_range3[0]) & (wl<wl_range3[1]))
    sel_norm3=(((wl>wl_norm3[0]) & (wl<wl_norm3[1])) | ((wl>wl_norm3[2]) & (wl<wl_norm3[3])))   
    
    sel_wl4=((wl>wl_range4[0]) & (wl<wl_range4[1]))
    sel_norm4=(((wl>wl_norm4[0]) & (wl<wl_norm4[1])) | ((wl>wl_norm4[2]) & (wl<wl_norm4[3])))   
    
    sel_wl5=((wl>wl_range5[0]) & (wl<wl_range5[1]))
    sel_norm5=(((wl>wl_norm5[0]) & (wl<wl_norm5[1])) | ((wl>wl_norm5[2]) & (wl<wl_norm5[3])))   
    
    sel_wl6=((wl>wl_range6[0]) & (wl<wl_range6[1]))
    sel_norm6=(((wl>wl_norm6[0]) & (wl<wl_norm6[1])) | ((wl>wl_norm6[2]) & (wl<wl_norm6[3])))   
    ...
    cm2inch = 1/2.54 
    #fig, axs=plt.subplots(2,2,figsize=(30*cm2inch,20*cm2inch), gridspec_kw=gs)
    fig = plt.figure(figsize=(40*cm2inch,30*cm2inch))
    #gs = fig.add_gridspec(2, 2, height_ratios=heights, width_ratios=widths, hspace=0.2, wspace=0.3)
    gs = fig.add_gridspec(3, 3, height_ratios=heights, width_ratios=widths, hspace=0.5, wspace=0.8)
    ...
    for i_model in range(0, N_row):
        ...
        _norm1=np.mean(matrix_spec[i_model, sel_norm1])
        _norm2=np.mean(matrix_spec[i_model, sel_norm2])
        _norm3=np.mean(matrix_spec[i_model, sel_norm3])
        _norm4=np.mean(matrix_spec[i_model, sel_norm4])
        _norm5=np.mean(matrix_spec[i_model, sel_norm5])
        _norm6=np.mean(matrix_spec[i_model, sel_norm6])
        ...
        #axs[1,0].plot(wl[sel_wl], spec[idx_sel[i_model], sel_wl]/_norm-spec_sel[sel_delta_min, sel_wl]/norm, alpha = 0.1,linewidth = 0.1, color=c)
        ax1.plot(wl[sel_wl1], matrix_spec[i_model, sel_wl1]/_norm1, alpha = 0.1,linewidth = 0.05, color=c)
        ax2.plot(wl[sel_wl2], matrix_spec[i_model, sel_wl2]/_norm2, alpha = 0.1,linewidth = 0.05, color=c)
        
        ax4.plot(wl[sel_wl3], matrix_spec[i_model, sel_wl3]/_norm3, alpha = 0.1,linewidth = 0.05, color=c)
        ax5.plot(wl[sel_wl4], matrix_spec[i_model, sel_wl4]/_norm4, alpha = 0.1,linewidth = 0.05, color=c)
        ax6.plot(wl[sel_wl5], matrix_spec[i_model, sel_wl5]/_norm5, alpha = 0.1,linewidth = 0.05, color=c)
        ax7.plot(wl[sel_wl6], matrix_spec[i_model, sel_wl6]/_norm6, alpha = 0.1,linewidth = 0.05, color=c)
    ...
    ax6.set_xlabel(r'$\lambda [\AA]$', size=20)
    ax7.set_xlabel(r'$\lambda [\AA]$', size=20)

    ax6.set_ylabel(r'$L_{\lambda}/L_{\lambda,ref}$', size=20)
    ax4.set_ylabel(r'$L_{\lambda}/L_{\lambda,cont}$', size=20)
    ax1.set_ylabel(r'$L_{\lambda}/L_{\lambda,cont}$', size=20)
