    ...
    if nthreads lt maxthreads and i_chunk lt nchunks then begin ; still chunks to process and thread free
      ...
      for ithread=0,maxthreads-1 do begin
        if not(OBJ_VALID(obridge[ithread])) and i_chunk lt nchunks then begin
          ...
          obridge[ithread]=obj_new('IDL_IDLBridge',output=getenv('HOME')+'/idl/log_files/indexerrors_'+string(i_chunk+1, format='(I02)')+'.log') ; write terminal output to the named file
          ...
          obridge[ithread]->execute,'.r '+getenv('TESI_EDO')+'index_errors.pro' ;; fill in your idl .pro file to be compiled
          obridge[ithread]->execute,'index_errors,'+string(i_chunk+1)+','+string(SNR),/nowait  ;; fill in the command line to process a single chunk i_chunk
